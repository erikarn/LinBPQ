...
typedef struct DEST_LIST
{
	...
	q_head_t DEST_Q;				// QUEUE OF FRAMES FOR THIS DESTINATION
...
typedef struct PORTCONTROL
{
	...
	q_head_t PORTRX_Q;			// FRAMES RECEIVED ON THIS PORT
	q_head_t PORTTX_Q;			// FRAMES TO BE SENT ON THIS PORT
...
//	UINT * NEXTCHR;			// 	
	q_head_t ASYNCMSG_Q;		//  RECEIVED MESSAGES
	q_head_t KISSTX_Q	;		// MESSAGES TO SEND
...
//	UINT * IORXEA;	
	...
	q_head_t PCTX_Q;				// HDLC HOLDING QUEUE
...
//;
	...
	q_head_t	RX_Q;			// PACKETS RECEIVED ON THIS LINK
	q_head_t	TX_Q;			// PACKETS TO SEND
	q_head_t	FRAMES[8];		// FRAMES WAITING ACK
	q_head_t RXFRAMES[8];	// Frames received out of sequence
